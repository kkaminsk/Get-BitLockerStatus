...
- Creates a new subfolder named:
  - `BitLockerStatus-DD-MM-YYYY-HH-MM`
- Writes an activity log named `Get-BitLockerStatus.log`
...
- Exports event logs (EVTX):
  ...
  - (Optional) With `-ZIP`:
    - Creates a ZIP archive of the output folder at `<BaseFolder>\\BitLockerStatus-<date>-<time>.zip` (parent of the log root)
...
```
  ...
  # Uses Windows Temp
  .\Get-BitLockerStatus.ps1 -UseTemp

  # Custom log folder
  .\Get-BitLockerStatus.ps1 -OutputPath "D:\Support\Logs"

  # Include MDM diagnostics and use Windows temp folder
  .\Get-BitLockerStatus.ps1 -MDM -UseTemp

  # Create ZIP archive of outputs
  .\Get-BitLockerStatus.ps1 -ZIP

  # MDM + ZIP to Windows temp
  .\Get-BitLockerStatus.ps1 -MDM -ZIP -UseTemp
```

  ## Output structure (example)
  Without `-MDM`:
  ```
  <Documents or C:\Windows\Temp or OutputPath>\BitLockerStatus-DD-MM-YYYY-HH-MM\
    ...
    Get-BitLockerStatus.log - log for this script
  ```
  ...
  With `-ZIP`:
  - A ZIP archive named `<COMPUTERNAME>-BitLockerStatus-<date>-<time>.zip` is created in the parent folder of the log root (e.g., Documents or C:\\Windows\\Temp).
...
Use these markers to quickly determine outcomes in `Get-BitLockerStatus.log`.

Quick search example:

```powershell
Select-String -Path "<LogRoot>\Get-BitLockerStatus.log" -Pattern "STEP: MDM XML parsing|STEP: .* event log export|STEP: FVE registry export"
```
...
```powershell
Select-String -Path "<LogRoot>\Get-BitLockerStatus.log" -Pattern "STEP: MDM XML parsing|STEP: .* event log export|STEP: FVE registry export|STEP: ZIP archive"
```
