...
.DESCRIPTION
Implements the application specification in "Get-BitLockerStatus.ps1 App Specification.md".
Run this script in an elevated PowerShell session (Run as Administrator).
  ...
  - <COMPUTERNAME>-<BitLockerStatus-...>.zip (when -ZIP is used; archive is created in the parent folder of the log root)
   - Get-BitLockerStatus.log (activity log)
  ...
  Optional. Fully qualified path to the base folder where the timestamped BitLockerStatus-<date>-<time> folder will be created.
  ...
  .PARAMETER ZIP
  Optional switch. When present, creates a ZIP archive of the output folder named `<COMPUTERNAME>-<BitLockerStatus-...>.zip` in the parent folder of the log root.
   !#>
...
# Create folder structure: BitLockerStatus-DD-MM-YYYY-HH-MM
$timestamp = Get-Date -Format 'dd-MM-yyyy-HH-mm'
$folderName = "BitLockerStatus-{0}" -f $timestamp
$logRoot = Join-Path $baseDir $folderName
New-Item -ItemType Directory -Path $logRoot -Force | Out-Null

$logFile = Join-Path $logRoot 'Get-BitLockerStatus.log'
...
Write-Log "Get-BitLockerStatus started."
